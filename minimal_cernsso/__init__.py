...
def session_from_cookies_file(
    ...
    logger.debug(
        ...
        cert_file, key_file, cookie_filename
        )
    s = requests.Session()
...
def refresh_cookies_file(
    ...
    for cookie in s.cookies:
        ...
        logger.debug(
            ...
            cookie.name, old_expires, cookie.expires
            )
...
def fresh_session(
    ...
    final_auth_url = "{0}?{1}".format(
        urljoin(r1.url, CERT_AUTH_URL_FRAGMENT),
        redirect_url.query
        )
    logger.info("Request #2, performing SSL Cert authentication against %s", final_auth_url)
    r2 = s.get(
        final_auth_url,
        cookies=cookiejar,
        verify=False,
    ...
    form_data = { elm.get('name') : elm.get('value') for elm in tree.findall("body/form/input") }
...
def cli_get_cookies():
    ...
    parser.add_argument('-o', '--out', type=str, default='cookies.txt', help='Path to the output cookies file (default=%(default))')
